...
async def set_censorship(katsu: KatsuService) -> None:
    reject_query_if_not_permitted()
    await get_censorship_settings_for_this_request(katsu)
    await reject_if_too_many_filters(katsu)


async def censorship_settings_lookup(katsu: KatsuService) -> tuple[int, int]:
    ...
    max_filters, count_threshold = await katsu.katsu_censorship_settings(project_id=project_id, dataset_id=dataset_id)
...
async def get_censorship_settings_for_this_request(katsu: KatsuService) -> None:
    ...
    g.max_filters, g.count_threshold = await censorship_settings_lookup(katsu)


async def threshold_retry(katsu: KatsuService) -> int:
    _, count_threshold = await censorship_settings_lookup(katsu)
    g.count_threshold = count_threshold
    return count_threshold


async def max_filters_retry(katsu: KatsuService) -> int:
    max_filters, _ = await censorship_settings_lookup(katsu)
    g.max_filters = max_filters
    return max_filters


async def get_censorship_threshold(katsu: KatsuService) -> int:
    user_permissions = g.permissions
    if has_full_record_permissions(user_permissions):
        return 0
    threshold = g.get("count_threshold")
    return threshold if threshold is not None else await threshold_retry(katsu)


async def censored_count(count: int, katsu: KatsuService) -> int:
    t = await get_censorship_threshold(katsu)
    if count <= t:
        return 0
    return count
...
async def get_max_filters(katsu) -> int:
    max_filters = g.get("max_filters")
    return max_filters if max_filters is not None else await max_filters_retry(katsu)


# ugly side-effect code, but keeps censorship code together
async def reject_if_too_many_filters(katsu: KatsuService) -> None:
    user_permissions = g.permissions
    if has_full_record_permissions(user_permissions):
        return
    max_filters = await get_max_filters(katsu)
...
async def censored_chart_data(data, katsu: KatsuService) -> list[dict[str, int]]:
    t = await get_censorship_threshold(katsu)  # zero with correct permissions
