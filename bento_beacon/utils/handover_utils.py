...
async def filenames_by_results_set(katsu: KatsuService, ids, project_id, dataset_id):
    if not ids:
        return {}
    ...
    response = await katsu.post(payload, project_id=project_id, dataset_id=dataset_id)
...
async def handover_for_ids(katsu: KatsuService, ids, project_id, dataset_id):
    # ideally we would preserve the mapping between ids and links,
    # but this requires changes in katsu to do well

    handovers = {}

    files_for_results = await filenames_by_results_set(katsu, ids, project_id, dataset_id)
