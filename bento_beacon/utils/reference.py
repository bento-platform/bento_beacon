...
async def gene_position_lookup(gene_id: str, assembly_id: str) -> dict[str, str | int | None]:
    reference_url = await get_service_url_or_raise("reference", current_app.logger)
    reference_gene_search_url = f"{reference_url}/genomes/{assembly_id}/features?name={gene_id}"
    try:
        async with aiohttp.ClientSession(connector=tcp_connector(current_app.config)) as s:
            async with s.get(reference_gene_search_url, headers=await create_access_header_or_fall_back()) as r:
                if not r.ok:
                    current_app.logger.warning(f"reference service error, status: {r.status}, message: {r.text}")
